...
public class GetFileListService {
	...
	public GetFileListResult getFileList(GetFileListParam getFileListParam) {
		...
		try {
			...
			for (ListBlobItem blobItem : container.listBlobs(DemoFuncConst.BLOB_FILE_PATH, true)) {
				// ファイル名・ファイルパスを取得
				String filePath = blobItem.getUri().toString();
				String fileName = filePath.substring(filePath.lastIndexOf("/") + 1);

				// Blobデータを読み込み
				CloudBlockBlob blob = container.getBlockBlobReference(DemoFuncConst.BLOB_FILE_PATH + fileName);
			...
			result.setAzureFunctionsMessage("Azure Functionsのメッセージ１");
